...
NOTE: Veeam requires a GCVE solution user account with elevated privileges when adding the GCVE vCenter server to the Veeam Backup and Replication inventory. For more information refer to the Google Cloud Platform (GCP) documentation at https://cloud.google.com/vmware-engine/docs/private-clouds/classic-console/howto-elevate-privilege[Elevating VMware Engine privileges].
...
video::6cb79743-e2fd-4bc3-a42d-b1ee0189865e[panopto, "Video walkthrough of the solution"]