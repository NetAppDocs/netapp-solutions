...
Click on the “New Private Cloud” button and enter the desired configuration for the GCVE Private Cloud. On “Location”, make sure to deploy the private cloud in the same Region/Zone where GCNV/CVO is deployed, to ensure the best performance and lowest latency.
...
[%collapsible]
====
Refer link:gcp-ncvs-datastore.html[Procedure to deploy supplemental NFS datastore with GCNV to GCVE]
====