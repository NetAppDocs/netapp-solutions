...
When using LDAP and Kerberos, you should determine the network ports in use by these services. You can find a complete list of ports in use by Cloud Volumes Service in the https://cloud.google.com/architecture/partners/netapp-cloud-volumes/security-considerations[Cloud Volumes Service documentation on security considerations^].
