...
. In the *Default Volume Configuration* section, specify the volume name and size required for datastore and click *Next*. This should be an NFSv3 volume. For *Storage Efficiency*, choose *Enabled* to turn on the ONTAP storage efficiency features (compression, deduplication, and compaction). After creation, use the shell to modify the volume parameters using *_vol modify_* as follows:
+
[%autowidth.stretch]
|===
...
+
*Cmd to create new datastore volume from shell:*
+
  Vol create -vserver FSxONTAPDatastoreSVM -volume DemoDS002 -aggregate aggr1 -size 1024GB -state online -tiering-policy snapshot-only -percent-snapshot-space 0 -autosize-mode grow -snapshot-policy none -junction-path /DemoDS002
+
*Note:* The volumes created via shell will take few minutes to show up in the AWS Console.
+
*Cmd to modify volume parameters which are not set by default:*
+
