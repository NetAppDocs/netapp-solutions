...
Veeam Explorer for Oracle database provides the ability to perform a standard Oracle database restore or an uninterrupted restore using Instant Recovery. It also supports recovery of Data Guard databases and restores from RMAN backups. In this section an Oracle database restore to a different server is covered using Veeam Explorer.
...
+
image::aws-vmc-veeam-28.png[Click on Browse to launch Veeam Explorer]

. In the Restore Wizard specify the restore point to restore from and click *Next*.
+
image::aws-vmc-veeam-29.png[Click on Browse to launch Veeam Explorer]

. Specify the target server the database will be restored to and the account credentials and click *Next*.
+
image::aws-vmc-veeam-30.png[Click on Browse to launch Veeam Explorer]
...
. Finally, specify the database files target location and click the *Restore* button to start the restore process.
+
image::aws-vmc-veeam-31.png[Click on Browse to launch Veeam Explorer]
