...
....
PS C:\> nslookup GCNV-EAST-433D
Server:  activedirectory. region. lab. internal
Address:  10. xx.0. xx
Name:    GCNV-EAST-433D.cvsdemo.local
Address:  10. xxx.0. x
...
* *Make snapshot directory visible (available for both GCNV-Performance and GCNV-SW).* This option controls whether or not SMB clients can access the Snapshot directory in an SMB share (`\\server\share\~snapshot` and/or Previous Versions tab). The default setting is Not Checked, which means that the volume defaults to hiding and disallowing access to the `~snapshot` directory, and no Snapshot copies appear in the Previous Versions tab for the volume.
