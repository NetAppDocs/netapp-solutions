...
* If Windows names and UNIX names do not match identically, then LDAP must be configured to allow Cloud Volumes Service to use the LDAP name mapping configuration (see the section link:ncvs-gc-other-nas-infrastructure-service-dependencies.html#_using_ldap_for_asymmetric_name_mapping[“Using LDAP for asymmetric name mapping”]).
