...
You can only enable LDAP for `GCNV-Performance` instances and use it for NFSv3, NFSv4.1, or dual-protocol volumes. An Active Directory connection must be established in the same region as the Google Cloud NetApp Volumes volume for successful deployment of the LDAP-enabled volume.
...
|Defines what level of session security is used by LDAP (sign, seal, or none). LDAP signing is supported by GCNV-Performance, if requested by Active Directory. GCNV-SW does not support LDAP signing. For both service types, sealing is currently not supported.
|None
|No
