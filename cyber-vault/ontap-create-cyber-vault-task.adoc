...
+
[NOTE]
Service providers should consider customers' contract end dates when determining retention period. For example, if the cyber vault retention period is 30 days and the customer contract ends before the retention expires, data in the cyber vault can not be deleted until the retention period ends.
+
