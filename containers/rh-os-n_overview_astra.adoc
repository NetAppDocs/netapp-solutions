...
. Add a DNS entry in the DNS server pointing the `astraAddress` as provided in the Astra Control Center CRD file to the `EXTERNAL-IP` of the traefik service.
+
image:redhat_openshift_image122.jpg[Add DNS entry for ACC GUI]

.	 Log into the Astra Control Center GUI by browsing to the FQDN you provided in the CRD file.
+
