---
sidebar: sidebar
permalink: containers/vtwn_overview_netapp.html
keywords: NetApp, ONTAP, Element, Storage, Tanzu, VMware Tanzu with NetApp ONTAP
summary: NetApp has several storage platforms that are qualified with Trident and Trident Protect to provision, protect and manage data for containerized applications and thus help in defining and maximizing DevOps throughput.
---

= NetApp storage systems overview
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
:imagesdir: ../media/
:solution_name: VMware Tanzu with NetApp
:ontap_page_link: vtwn_netapp_ontap.html

[.lead]
NetApp has several storage platforms that are qualified with Trident and Trident Protect to provision, protect and manage data for containerized applications and thus help in defining and maximizing DevOps throughput.

include::../_include/containers_common_intro_sections.adoc[tags=netapp_overview_page;!netapp_overview_page_element]
