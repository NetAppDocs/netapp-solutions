...
. Add NetApp Astra Trident helm repository.
+
----
...
. Create a new namespace to install Trident in it.
+
----
...
. For user or workload clusters managed by TKGS (vSphere with Tanzu) or TKG with management cluster deployments, follow the below procedure to install Astra Trident:
...
. For user or workload cluster managed by TKGI deployments, run the below helm command to install Trident operator in the namespace created.
+
----
