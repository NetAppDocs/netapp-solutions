...
= NetApp ONTAP: OpenShift on Red Hat Virtualization
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
NetApp FAS/AFF are best in class storage systems that have been designed to provide enterprise-class, multi-protocol, scale-out hybrid (FAS) and all-flash (AFF) storage.

Both systems are powered by NetApp ONTAP data management software, the industry’s most advanced data-management software for highly-available, cloud-integrated, simplified storage management.
...
link:rh-os-n_overview_trident.html[Next: NetApp Storage Overview]
