...
NetApp Astra Control Center can be installed on a Red Hat OpenShift cluster that has the Trident storage orchestrator deployed and configured with storage classes and storage backends to NetApp ONTAP storage systems.

For the installation and configuration of Trident to support Astra Control Center, see link:rh-os-n_overview_trident.html[this document^].
...
.	Trident must already be installed and configured on each Red Hat OpenShift cluster.
...
.	A default StorageClass configured on each OpenShift cluster with Trident as the storage provisioner.
