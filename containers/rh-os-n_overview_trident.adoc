...
When using NetApp ONTAP as backend:
[source, cli]
...
When using NetApp Element as backend:
[source, cli]
apiVersion: machineconfiguration.openshift.io/v1
kind: MachineConfig
metadata:
...
[start=2]
. After the configuration is created, it will take approximately 20–30 minutes to apply the configuration to the worker nodes and reload them.  Verify whether the machine config is applied by using `oc get mcp` and make sure that the machine config pool for workers is updated. You can also log in to the worker nodes to confirm that the iscsid service is running when using NetApp Element as backend or iscsid and multipathd services are running when using NetApp ONTAP as backend.
+
