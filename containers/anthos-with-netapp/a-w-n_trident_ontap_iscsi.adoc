...
----
+
NOTE: There is an optional field called `fsType` that is defined in this file. In iSCSI backends, this value can be set to a specific Linux filesystem type (XFS, ext4, and so on), or it can be deleted to allow OpenShift to decide which filesystem to use.
...
link:a-w-n_trident_element_iscsi.html[Next: NetApp Element iSCSI.]
