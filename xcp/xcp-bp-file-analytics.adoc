---
sidebar: sidebar
permalink: xcp-bp-file-analytics.html
...
[NOTE]
For more information about the high-level architecture of XCP file analytics, GUI-based dashboard views such as stats view, and file distribution view details, see the https://blog.netapp.com/xcp-data-migration-software[NetApp XCP 1.6 Delivers Open File Analytics and Infrastructure Improvements^] blog.

In XCP 1.6, there is a limited GUI for customized graphs. To create the required graphs, you can leverage the CLI by running the `xcp scan` command with matching filters. See the following examples.
