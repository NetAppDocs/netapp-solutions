---
sidebar: sidebar
permalink: saphana_aff_nfs_storage_controller_setup.html
keywords:
summary:
---
...
. Check the correct disk shelf configuration as described in “.”
. Create and configure the required aggregates as described in “.” 
. Create a storage virtual machine (SVM) as described in “.”
. Create LIFs as described in “.”
. Create volumes within the aggregates as described in “” and “.”
. Set the required volume options as described in “.”
. Set the required options for NFSv3 as described in “” or for NFSv4 as described in “. ”
. Mount the volumes to namespace and set export policies as described in “. ”

=== Disk shelf connection

==== SAS disk shelves

A maximum of one disk shelf can be connected to one SAS stack to provide the required performance for the SAP HANA hosts, as shown in . The disks within each shelf must be distributed equally to both controllers of the HA pair.  ADPv2 is used with ONTAP 9 and the DS224C disk shelves.
...
Each NS224 NVMe disk shelf is connected using two 100GbE ports per controller. The disks within each shelf must be distributed equally to both controllers of the HA pair. ADPv2,  as described in the aggregate configuration chapter,  is also used for the NS224 disk shelf.
...
The maximum amount of data and log volume mounts per physical interface are shown in .
...
=== Volume configuration for SAP HANA single-host systems
 ...
 shows the volume configuration of four single-host SAP HANA systems. The data and log volumes of each SAP HANA system are distributed to different storage controllers. For example, volume `SID1_data_mnt00001` is configured on controller A, and volume `SID1_log_mnt00001` is configured on controller B.
...
For each SAP HANA host, a data volume, a log volume, and a volume for `/hana/shared` are configured.  shows an example configuration for single-host SAP HANA systems.
...
|===
 ...
 shows an example of the mount point configuration for a single-host system.  To place the home directory of the `sidadm` user on the central storage,  the `/usr/sap/SID` file system should be mounted from the `SID_shared` volume.
...
=== Volume configuration for SAP HANA multiple-host systems
 ...
 shows the volume configuration of a 4+1 SAP HANA system. The data and log volumes of each SAP HANA host are distributed to different storage controllers. For example, volume `SID1_data1_mnt00001` is configured on controller A, and volume `SID1_log1_mnt00001` is configured on controller B.
...
For each SAP HANA host, a data volume and a log volume are created. The `/hana/shared` volume is used by all hosts of the SAP HANA system.  shows an example configuration for a multiple-host SAP HANA system with four active hosts.
...
|===
 ...
 shows the configuration and the mount points of a multiple-host system with four active SAP HANA hosts.  To place the home directories of the `sidadm` user of each host on the central storage,  the `/usr/sap/SID` file systems are mounted from the `SID_shared` volume.
...
You must verify and set the volume options listed in  on all SVMs. For some of the commands, you must switch to the advanced privilege mode within ONTAP.
...
The NFS options listed in  must be verified and set on all storage controllers.  For some of the commands shown in , you must switch to the advanced privilege mode.
...
The NFS options listed in  must be verified and set on all SVMs.

For some of the commands in , you must switch to the advanced privilege mode.
...
Set the NFSv4 lease time at the SVM as shown in  if SAP HANA multiple host system are used.
...
The parameters are within `nameserver.ini` as shown in . Keep the default retry interval of 10 seconds within these sections.
