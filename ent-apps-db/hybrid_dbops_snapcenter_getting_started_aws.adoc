...
. Navigate to https://cloud.netapp.com/cloud-manager[NetApp Cloud Central^] and login or sign up.
...
Following lists additional configuration steps after an EC2 Linux instance is initially deployed.

. Add Oracle standby instance to DNS server for name resolution within the SnapCenter management domain

. Add a Linux management user ID as SnapCenter OS credential with sudo permission without password. Enable the ID with ssh password authentication on EC2 instance. (By default, ssh password authentication and passwordless sudo is turned off on EC2 instances)

. Configure Oracle installation to match with on-prem Oracle installation such as OS patches or Oracle version and patches etc.

. NetApp Ansible DB automation roles can be leveraged to configure EC2 instance for database DEV/TEST and DR use cases. The automation code can be download from NetApp public GitHub site. https://github.com/NetApp-Automation/na_oracle19c_deploy[Oracle 19c Automated Deployment^]. The goal is to install and configure database software stack on EC2 instance to match on-prem OS and database configuration for the use cases.

==== Windows Instance Configuration for SQL Server Workload

Following lists additional configuration steps after an EC2 Windows instance is initially deployed.

. Retrieve Windows administrator password to login instance via RDP

. Disable Windows firewall, join the host to Windows SnapCenter Domain, and add the instance to DNS server for name resolution

. Provision a SnapCenter log volume to store SQL Server log files

. Configure iSCSI on Windows host to mount the volume and format the disk drive

. Again, many of the above tasks can be automated with NetApp automation solution for SQL Server. Check NetApp automation public GitHub site for newly published roles and solutions. https://github.com/NetApp-Automation[NetApp-Automation^] 
