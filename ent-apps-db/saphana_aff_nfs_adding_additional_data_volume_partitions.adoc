---
sidebar: sidebar
permalink: saphana_aff_nfs_adding_additional_data_volume_partitions.html
keywords:
summary:
---
...
|===
 ...
 shows an example of the mount point configuration for a single-host system with two data volume partitions.
...
The layout of volumes is like the layout for a multiple- host SAP HANA system with one data volume partition but with an additional data volume stored on a different aggregate as log volume and the other data volume.  shows an example configuration of an SAP HANA multiple-host system with two data volume partitions.
...
In addition to the tasks described in the section “”,  the additional mount points and `fstab` entries for the new additional data volume/s need to be created and the new volumes need to be mounted.
...
+
Below is an example for a single-host system using NFSv4.1:
