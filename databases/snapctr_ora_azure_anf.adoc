...
* Backup and recovery Oracle database deployed on ANF in Azure cloud with SnapCenter  
...
* *Separate full database and archive log backup.* Full database backup includes data volumes and log volumes consistent group snapshots. A full database snapshot incurs higher storage consumption and should execute less frequently than an archive log-only backup, which consumes less storage and improves RPO. There is also a limit (1023) on the number of snapshot backups on a volume.

* *A.* ANF file system IO throughput is regulated based on the size of volume. For initial deployment, allocate large size volumes can speed up the deployment. The volumes subsequently can be downsized dynamically without application impact.
