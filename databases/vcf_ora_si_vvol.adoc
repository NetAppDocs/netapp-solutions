...
. From DB VM ora_01, validate that the test table is recovered after a successful restore/recovery and database is rolled forward to most recent state.
+
.....
...
In this example, the most recent backup sets is used to clone a database on the DB VM ora_02 in a different ORACLE_HOME in VCF. 
