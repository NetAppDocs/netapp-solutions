...
. Log into the EC2 instance via SSH as the ec2-user. Then change to your SSH key and EC2 instance IP address.
+
....
...
. As the Oracle user, launch `gridSetup.sh` for grid infrastructure installation.
+
....
...
. If you received the message "[INS-43080] Some of the configuration assistants failed, were cancelled or skipped", run the following command as oracle user to check if the DATA disk group resource was created.
+
....
bin/crsctl stat res -t
....

. If the DATA disk group was not created, reload the multipathd as root user.
+
....
systemctl restart multipathd
....

. As the oracle user, create a DATA disk group if it was not created during grid installation. 
+
....
...
. Execute silent software-only DB installation.
+
....
 ./runInstaller -applyRU /tmp/archive/34765931/ -silent -ignorePrereqFailure -responseFile /tmp/archive/dbinstall.rsp
....

. Run the `root.sh` script after sofware-only installation.
+
....
/u01/app/oracle/product/19.0.0/db1/root.sh
....

. Create the `dbca.rsp` file with the following entries:
+
....
