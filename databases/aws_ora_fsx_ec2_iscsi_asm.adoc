...
. Create a staging directory /tmp/archive folder and set 777 permission
+
[source, cli]
mkdir /tmp/archive
+
...
. Download and stage Oracle binary installation files and other required rpm files to /tmp/archive directory
+
...
. Install Oracle 19c preinstall RPM, which will satisfy most kernel configuration requirement
+
[source, cli]
...
. Add following lines to limit.conf to set file descriptor limit and stack size without quote " "
+
[source, cli]
...
. Change node.session.timeo.replacement_timeout in iscsi.conf configuration file from 120 to 5 seconds
+
[source, cli]
...
sudo vi /etc/fstab
+
add this line "UUID=d89fb1c9-4f89-4de4-b4d9-17754036d11d       /u01    xfs     defaults,nofail 0       2"
+
[NOTE]
+
It is important to mount binary with only UUID and with nofail option to avoid possible root lock issue during EC2 instance reboot.

. As root user, add udev rule for oracle devices
+
[source, cli]
...
. As root user, reload the udev rules
+
[source, cli]
udevadm control --reload-rules

. As root user, trigger the udev rules
+
[source, cli]
udevadm trigger

. As root user, reload multipathd
+
[source, cli]
...
+
ignore the warnings about wrong groups for grid infrastructure. We are using single oracle user to manage Oracle Restart, this is to be expected. 
...
. If you received message "[INS-43080] Some of the configuration assistants failed, were cancelled or skipped", as oracle user, run following command to check if DATA disk group resource is created.
+
[source, cli]
bin/crsctl stat res -t

. If DATA disk group not created, as root user, reload the multipathd
+
[source, cli]
systemctl restart multipathd

. As oracle user, create DATA disk group if not created during grid installation. 
+
[source, cli]
...
====
. Login as oracle user and unset $ORACLE_HOME, $ORACLE_SID if set
+
[source, cli]
unset ORACLE_HOME
unset ORACLE_SID

. Create oracle DB home directory and change to it
+
[source, cli]
mkdir /u01/app/oracle/product/19.0.0/db1
cd /u01/app/oracle/product/19.0.0/db1

. Unzip oracle DB installation files
+
[source, cli]
unzip -q /tmp/archive/LINUX.X64_193000_db_home.zip

. From DB home, delete OPatch directory
+
[source, cl]
rm -rf OPatch

. From DB home, copy p6880880_190000_Linux-x86-64.zip to grid_home, then unzip it
+
[source, cli]
cp /tmp/archive/p6880880_190000_Linux-x86-64.zip .
unzip p6880880_190000_Linux-x86-64.zip

. From DB home, revise cv/admin/cvu_config, uncomment and replace "CV_ASSUME_DISTID=OEL5" to "CV_ASSUME_DISTID=OL7"
+
[source, cli]
vi cv/admin/cvu_config

. From /tmp/archive directory, unpack DB 19.18 RU patch
+
[source, cli]
unzip p34765931_190000_Linux-x86-64.zip

. Prepare DB silent install rsp file in /tmp/archive/dbinstall.rsp directory with following values
+
...
. Execute silent softwareonly DB installation
+
[source, cli]
 ./runInstaller -applyRU /tmp/archive/34765931/ -silent -ignorePrereqFailure -responseFile /tmp/archive/dbinstall.rsp

. Run root.sh script after sofware only installation
+
[source, cli]
/u01/app/oracle/product/19.0.0/db1/root.sh

. create dbca.rsp file with following entries:
+
...
. Validate Oracle Restart HA services after DB creation
+
...
. Set DB recovery location to +LOGS disk group
+
[source, cli]
...
. Login database with sqlplus and enable archive log mode 
+
[source, cli]
sqlplus /as sysdba
+
...
This would complete Oracle 19c version 19.18 Restart deployment on AWS FSx ONTAP and EC2 compute instance. If desired, it is recommended to relocate Oracle control file and online log files to +LOGS disk group. 
...
Refer to link:snapctr_svcs_ora.html[SnapCenter Services for Oracle^] for details on Oracle database backup, restore, and clone with NetApp BlueXP console.
