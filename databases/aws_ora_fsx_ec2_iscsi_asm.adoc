...
. Prepare a gridsetup.rsp file for silent installation and place the rsp file in grid home directory. The rsp file should cover section A,B and G with following infomation:
+
...
+
[source, cli]
./gridSetup.sh -applyRU /tmp/archive/34762026/ -silent -responseFile gridsetup.rsp
