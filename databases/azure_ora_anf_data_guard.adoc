...
. Once Oracle software is installed and configured, set oracle home and path. Also, from standby $ORACLE_HOME dbs directory, copy oracle password from primary database if you have not done so.
+
[source, cli]
...
In the event of a failure, Data Guard Broker can be used to failover primary database to standby instantaniouly. If `Fast-Start Failover` is enabled, Data Guard Broker can failover primary database to standby when a failure is detected without an user intervention.
