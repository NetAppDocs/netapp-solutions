...
= TR-XXX: Quick Recovery of Oracle VLDB with Incremental Merge on ANF
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
* Quick recovery of an Oracle VLDB in the event of failure on the same Azure database server VM.
* Quick recovery of an Oracle VLDB in the event of failure on a standby Azure database server VM.
...
* A DBA who set up Oracle VLDB image copy incremental merge via RMAN in Azure for faster database recovery.
...
image::aws_ora_fsx_ec2_vldb_architecture.png["This image provides a detailed picture of the Oracle VLDB incremental merge implementation in Azure public cloud with ANF."]
...
[source, cli]
alter database enable block change tracking using file '/nfsanf/oracopy/bct_db1.ctf'
+
[source, cli]
...
. As oracle user, launch RMAN to run Oracle recovery on the standby DB server host.
+
....
...
. Validate the database structure restored to new host as well as the row we have inserted before primary VLDB failure. 
+
....
