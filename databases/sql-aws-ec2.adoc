...
== Setting up for SQL Server Operations to Perform Backup, Restore and Clone with SnapCenter

In order to perform SQL server operations with SnapCenter, you must first create volumnes and LUNs for SQL server.
...
.Appendix B: Workaround for restore failues (to a single LUN)
[%collapsible]
====
...
.Appendix C: Powershell scripts for provisioning volumes and LUNs
[%collapsible]
====
