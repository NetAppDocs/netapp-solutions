...
====
Below is an example of typical global variable file vars.yml that include variables that applicable at the global level. 
...
Host variables are defined in host_vars directory named as {{ host_name }}.yml that apply to the particular host only. For this solution, only target clone DB host parameters are configured. Oracle standby DB parameters are configured in global vars file. Below is an example of target Oracle clone DB host variable file orac.yml that shows typical configuration.
...
For an additional clone database, create a separate clone_n_asm_anf.yml and clone_n_asm_anf.sh. Configure the Ansible target hosts, global vars.yml, and hostname.yml file in host_vars directory accordingly.
