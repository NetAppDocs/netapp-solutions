...
* Oracle database data files image copy backup via RMAN to an in-server NFS file system on a Google Cloud NetApp Volumes storage pool.  
* Quick recovery of an Oracle VLDB in the event of a failure on the same or a standby Google Compute Engine VM.
* Clone an Oracle database copy on a clone host for other use cases from the staging DB image copy in minutes.
...
| Google Compute Engine VM for DB server | N1 (4 vcpus, 15 GiB memory) | 2 VMs, one as primary DB server and the other as a standby or clone DB server 
...
* *GCNV storage pool service level.* GCNV storage pool offers four service levels: Standard, Premium, Extreme, and Flex. By default, an auto QoS applies to a volume created within a storage pool, which restricts the throughput on the volume based on its size as well as the service level. The only exception is that volumes allocated from Flex storage pool share the throughput available to the storage pool. 

* *Oracle VLDB in-server backup storage limit for RMAN incremental merge.* In our test and validation, a NFS volume for Oracle incremental backup and merge is allocated from a single GCNV storage pool with standard service level, which has an 200 TiB capacity limit. For deployment over that thresholds, multiple GCNV storage pools can be implemented in parallel with multiple NFS mount points to provide higher capacity. 

* *Oracle recoverability using RMAN incremental merge.* The RMAN in-server incremental backup and merge is generally executed at a user-defined frequency based on your RTO and RPO objectives. If there is a total loss of primary database file storage and/or archived logs, data loss can occur. The Oracle database can be recovered up to the last incremental backup available from the in-server database backup image copy. To minimize data loss, the Oracle flash recovery area needs to be set up on GCNV NFS backup mount point, and archived logs are backed up to the same NFS mount along with database image copy to roll forward the database to the last archived log, in the event of a failure. Thus, near-zero RPO can be achieved.  

* *Running Oracle VLDB off NFS image backup on GCNV storage pool.* At this point, reassigning database volume service level from standard to premium or extreme is not supported. However, the reassigning of service levels from premium to extreme is supported. To run the database on image copy for an extended period after a failure, it is recommended to allocate volume from the premium storage pool for database image copy backup.

* *Google Compute Engine VMs.* In the test and validation, we used N1 type of GCE VM instance as the Oracle database servers. There are other types of GCE VMs that may be more optimized and better suited for database workload. Size the VM appropriately for the number of vCPUs and the amount of RAM based on actual workload requirements.
...
The solution assumes that an Oracle VLDB is already deployed in the Google Cloud environment within a VPC. The Oracle VLDB can run on either the GCNV storage or any other storage options within the Google Cloud ecosystem. The following section provides step-by-step deployment procedures for setting up an Oracle VLDB image backup and incremental merge via RMAN to a NFS file system mounted from a GCNV storage pool. The Oracle VLDB also has its primary storage hosted on a different GCNV storage pool.   
...
. From the Google Cloud portal console, follow documentation link:https://cloud.google.com/netapp/volumes/docs/configure-and-use/volumes/create-volume[Create a volume^] to create a volume for in-server Oracle VLDB image backup. In this solution demo, the Oracle VLDB primary storage is also hosted on GCNV as shown below.
+
...
=== Mount a NFS volume from GCNV to the primary Oracle VLDB server
[%collapsible%open]

====

The GCNV volume provisioned for the Oracle VLDB in-server backup ideally should be situated in the same Google Cloud zone as the primary DB server and the primary storage for better performance. 
...
. Login to the primary Oracle VLDB server. As root, mount the NFS volume exported from GCNV storage. Change to your NFS server IP address and file path as needed. 
+
[source, cli]
sudo mkdir /nfsgcnv
+
...
. Change mount point ownership to oracle:oisntall, or change to your oracle user name and primary group as necessary.
+
[source, cli]
...
. For this solution validation, the primary database is setup as a standalone server with grid infrastructure Restart configuration as shown below. The primary storage is hosted on GCNV storage pool with premium service. Again, this is not a requirement and the primary storage can be on any other storage option in Google cloud.
+
....
...
. List database image copy after backup to observe that a database image copy has been created in GCNV NFS mount point.
+
....
...
This completes the setup of an Oracle VLDB standby image copy backup and merge on an in-server NFS mount from GCNV storage.
...
. Insert a row into the test table. The row is inserted after a most recent incremental backup and merge and its data is not in the backup set. The subsequent demonstration would show that the test row is recovered after database is completely recovered.  
+
....
...
. As the oracle user, connect to Oracle database via RMAN to switch database to copy.
+
....
...
. From SQL plus, validate that the test row which we have inerted after the last incremental backup and before switching over to image backup copy has been recovered.
+
....
...
. Also, configure oracle environment on the standby DB server similiarly to the primary DB server, such as oratab, and oracle user .bash_profile etc. It is a good practice to backup those files to GCNV NFS mount point.
...
. As oracle user, launch RMAN to run Oracle recovery on the primary or standby DB server host. First, start the Oracle instance in `nomount` state.
+
....
...
. To reverse back to primary storage, follow exactly the same procedures as detailed in section `Reverse VLDB back to primary storage from backup copy`. If recovering on a standby DB server, the primary storage needs to be imported into standby DB server. There is also a need to relocate Oracle control files and current online log files from GCNV nfs mount back to the primary storage, which can be accomplished with few mins down time.
...
Clone a new Oracle VLDB from the backup image copy on GCNV is similar to the point-in-time recovery of the primary database to a new DB server. NetApp Solutions Engineering team provides an automation toolkit to facilitate the entire processes for a fast (mins) and effortless implementation. 

Following automation toolkit contains a playbook that can be used to create or refresh clones of an Oracle VLDB from a primary database backup image copy stored on GCNV storage via NFS mount for a complete clone lifecycle management.
