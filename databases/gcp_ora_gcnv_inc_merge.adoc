...
* *GCNV storage pool service level.* GCNV storage pool offers four service level: Standard, Premium, Extreme, and Flex. By default, an auto QoS applies to a volume created within a storage pool, which restricts the throughput on the volume based on its size as well as the service level. The throughput on a volume can also be manually set with manual QoS setting. 
