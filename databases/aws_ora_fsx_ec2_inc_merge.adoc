...
Recovery of an Oracle VLDB ( Very Large Database ) via Oracle RMAN  ( Recovery Manager ) backup tool can be a very challenging task. It could take long time to restore an Oracle VLDB from backup media in the event of a failure before database can be recovered. Your SLA could suffer a major hit as a result. Since version 10g, Oracle released a RMAN feature to allow Oracle user to stage image copy of an Oracle database data file at an extra disk storage on DB server host. This image copy of the data file can be incrementally updated via RMAN on the daily basis. In the event of a failure, DBA can switch Oracle database quickly from failed storage media to this image copy as database media restore is avoided. The net result is much improved SLA at the expense of doubling of database storage.   
...
* Oracle VLDB data file image copy incremental merge via RMAN on NFS mount point off AWS FSx ONTAP storage.  
* Quick recovery of Oracle VLDB by switching to database data file image copy on FSx ONTAP storage in the event of a failure.
* Clone FSx NFS file system volume storing VLDB data file image copy to be used for standing up a concurrent database copy for any other use cases. 

== Audience

This solution is intended for the following people:

* A DBA who would like to setup Oracle VLDB data file image copy incremental merge via RMAN for faster database recovery.
