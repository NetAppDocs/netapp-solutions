...
For Azure VMs deployed with local disk redundancy, ensure that you have allocated at least 128G in VM root disk to have sufficient space to stage Oracle installation files and add OS swap file. Ensure the database volume naming follows the VMname-u01, VMname-u02, and VMname-u03 convention.
...
+
video::81e389a0-d9b8-495c-883b-b0d701710847[panopto, title="Simplified and automated Oracle deployment on Azure NetApp Files with NFS", width=360]
...
. Execute playbooks one at a time with the number sequence from 1-5.
+
[source, cli]] 
