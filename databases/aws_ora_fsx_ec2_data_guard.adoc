...
Stop at step 10 in `Oracle database installation` section. The standby database copy will be instantiated from primary database instead. 

. Follow procedure link:https://docs.netapp.com/us-en/netapp-solutions/databases/aws_ora_fsx_ec2_iscsi_asm.html#provision-and-map-database-volumes-and-luns-to-the-ec2-instance-host[Provision and map database volumes and LUNs to the EC2 instance host^] to provision database volumes from FSx cluster to be presented to standby EC2 DB instance to host Oracle binary and standby database copy.

. Follow procedure link:https://docs.netapp.com/us-en/netapp-solutions/databases/aws_ora_fsx_ec2_iscsi_asm.html#database-storage-configuration[Database storage configuration] to configure storage on standby EC2 DB instance.

. First, logging to the FSx cluster through SSH as the fsxadmin user. Change to your FSx cluster management IP address, which can be retrieved from AWS FSx ONTAP UI console. 
+
[source, cli]
...
. Login to primary EC2 DB server as oracle user and validate the demo primary database db1 configuration. DB1 is setup with data files in ASM disk group +DATA and flash recovery area in ASM disk group +LOGS. 
+
....
[oracle@ip-172-30-15-45 ~]$ crsctl stat res -t
--------------------------------------------------------------------------------
Name           Target  State        Server                   State details
--------------------------------------------------------------------------------
Local Resources
...
. Enable "Force Logging" mode on primary database with following command via sqlplus.
+
[source, cli]
ALTER DATABASE FORCE LOGGING;

. Configure the primary database to receive redo data in the event of failover from primary to standby. The primary database can then quickly transition to the standby role and begin receiving redo data, if necessary. The standby redo log must have at least one more redo log group than the redo log for each redo thread at the redo source database. Repeat the command for each additional redo group.
+
[source, cli]
alter database add standby logfile thread 1 size 200M;
+
Validate the standby redo log groups created.
+
[source, cli]
...
---------- ---------- ---------- ----------
         4          1  209715200 UNASSIGNED
         5          1  209715200 UNASSIGNED
         6          1  209715200 UNASSIGNED
         7          1  209715200 UNASSIGNED
