...
.Restore VMDKs using ONTAP Tools
[%collapsible]
==== 
ONTAP Tools allows full restore of VMDK's to their original location or the ability to attach a VMDK as a new disk to the host. In this scenario a VMDK will be attached to Windows host in order to access the file system.
