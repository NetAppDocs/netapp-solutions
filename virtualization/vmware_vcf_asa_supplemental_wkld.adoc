...
=== Scenario Overview
In this scenario we will demonstrate how to deploy and use ONTAP Tools for VMware vSphere (OTV) to configure an vVols datastore for a VCF Workload Domain. iSCSI will be used as the storage protocol for the vVols datastore.
...
* Create vmkernel adapters for iSCSI on the VI Workload Domain ESXi hosts.
...
+
image:vmware-vcf-asa-image26.png[move uplink2 to unused]
