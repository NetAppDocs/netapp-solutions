...
* *vCenter UI extensions*  The ONTAP tools UI extensions simplify the jobs of the operations teams and vCenter admins by embeding easy to use context sensitive menus for managing hosts and storage, informational portlets, and native alerting capabilities directly in the vCenter UI for streamlined workflows.
