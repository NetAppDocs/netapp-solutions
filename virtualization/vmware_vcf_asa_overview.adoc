...
// Replace virtualized datacenter with SDDC? vSphere and vCenter combo enable virtualized datacenter.
...
//vDS is vCenter scoped. Overlay Transport Zone might be better fit.
...
//management domain can also have additional cluster. FC/IP switch is implied? Do we need the lines?
...
//Principal storage is deployed with SDDC manager as part of cluster creation orchestration.
...
//Supplemental storage is additional datastore provisioned to management or workload domain manually or using ONTAP Tools for NetApp Storage. Need to replace AFF with ASA?
...
//Generic guidance is not to use OTV on public documents. For short form, ONTAP tools is preferred.
...
//Need to include it uses storage based snapshots and replication for lower RTO?
...
//Need to include tag based protection?
...
For more information on 3-2-1 backup strategies with BlueXP visit https://docs.netapp.com/us-en/netapp-solutions/ehc/bxp-scv-hybrid-solution.html[3-2-1 Data Protection for VMware with SnapCenter Plug-in and BlueXP backup and recovery for VMs].
...
* Use Ontap Tools to deploy iSCSI datastores in a VCF management domain. Click https://docs.netapp.com/us-en/netapp-solutions/virtualization/vmware_vcf_asa_supp_mgmt_iscsi.html[*here*] for deployment steps.
* Use Ontap Tools to deploy vVols (iSCSI) datastores in a VI workload domain.. Click https://docs.netapp.com/us-en/netapp-solutions/virtualization/vmware_vcf_asa_supp_wkld_vvols.html[*here*] for deployment steps.
* Configure NVMe over TCP datastores for use in a VI workload domain. Click https://docs.netapp.com/us-en/netapp-solutions/virtualization/vmware_vcf_asa_supp_wkld_nvme.html[*here*] for deployment steps.
* Deploy and use the SnapCenter Plug-in for VMware vSphere to protect and restore VMs in a VI workload domain. Click https://docs.netapp.com/us-en/netapp-solutions/virtualization/vmware_vcf_asa_scv_wkld.html[*here*] for deployment steps.