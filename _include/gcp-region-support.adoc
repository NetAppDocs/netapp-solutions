...
[NOTE]
Only GCNV-Performance volumes can be used for GCVE NFS Datastore.
...
To minimize latency, NetApp Google Cloud NetApp Volumes (GCNV) and GCVE where you intent to mount the volume should be in same availability zone.
...
//|===
//| *GCP Region* | *GCVE Availability* | *GCNV Perf Availability* | *GCNV SW Availability* | *NFS Datastore Availability*
