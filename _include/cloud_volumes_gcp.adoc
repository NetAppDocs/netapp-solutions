...
.GCNV Volume
--
.Terraform configuration files for deployment of NetApp GCNV Volume on GCP

This section contains various Terraform configuration files to deploy/configure NetApp GCNV (Googlr Cloud NetApp Volumes Services) Volume on GCP (Google Cloud Platform).
...
Terraform variables for NetApp GCP GCNV Volume.

[width=100%, cols="20%, 10%, 70%", frame=none, grid=rows, options="header"]
|===
| *Name* | *Type* | *Description*
| *gcp_cvs_name* | String | (Required) The name of the NetApp GCNV volume.
| *gcp_cvs_project_id* | String | (Required) The GCP project_id where the GCNV Volume will be created.
| *gcp_cvs_gcp_service_account_path* | String | (Required) The local path of the service_account JSON file for GCP authorization purposes. This service account is used to create the GCNV Volume in GCP.
| *gcp_cvs_region* | String | (Required) The GCP zone where the GCNV Volume will be created.
