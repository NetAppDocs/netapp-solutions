...
Resizing the total storage capacity, volume size or LUN size for FSx for ONTAP requires manual intervention today. This document provides a step by step guide to create an automated setup that monitors FSx for ONTAP at regular intervals, resizes when a user-specified threshold is breached and notifies the end-user of the resizing activity.   
...
With the provided solution, it is easy to setup a monitoring solution that regularly monitors FSx for ONTAP Storage, resizes them based on user-specified threshold and provides an alerting mechanism. This makes the process of using and monitoring FSx for ONTAP seamless freeing up administrators to focus on business-critical activities while storage grows automatically when required.
