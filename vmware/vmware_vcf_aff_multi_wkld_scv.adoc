...
= VCF Multiple Workload Domains with NetApp SnapCenter 
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
image:vmware-vcf-aff-image50.png[VCF multi-workload domain with NetApp SnapCenter for vSphere Architecture]

SCV is deployed as a linux virtual appliance using an OVA file. This SnapCenter for VMWare vSphere plug-in  uses a remote plug-in architecture and runs outside the workload domain vCenter server and is hosted on management domain vCenter. Data that is on ONTAP FAS, AFF, or All SAN Array (ASA) primary systems and replicated to ONTAP FAS, AFF, or ASA secondary systems. SCV and VCF domain is one to one relationship and SCV provides fast, space-efficient, crash-consistent, and VM-consistent backup and restore operations for VMs, Datastores, and VMDKs.  VCF management domain and each VMware VCF workload domain requires one SCV.  It also works with SnapCenter Server to support application-based backup and restore operations in VMware environments for SnapCenter application-specific plug-ins. For more information check link:https://docs.netapp.com/us-en/sc-plugin-vmware-vsphere/index.html[SnapCenter Plug-in for VMware vSphere documentation.]

The 3-2-1 backup rule is a data protection strategy that involves making three copies of data, storing them on two different types of media, and keeping one copy off-site. BlueXP backup and recovery is a cloud based tool for data management that provides a single control plane for a wide range of backup and recovery operations across both on-premises and cloud environments. Part of the NetApp BlueXP backup and recovery suite is a feature that integrates with the SnapCenter Plugin for VMware vSphere (on-premises) to extend a copy of the data to object storage in the cloud. This establishes a third copy of the data offsite that is sourced from the primary or secondary storage backups. BlueXP backup and recovery makes it easy to set up storage policies that transfer copies of your data from either of these two on-prem locations. For detail, check link:https://docs.netapp.com/us-en/netapp-solutions/ehc/bxp-scv-hybrid-solution.html[3-2-1 Data Protection for VMware with SnapCenter Plug-in and BlueXP backup and recovery for VMs.]

== Deploy a VCF with  Management Domain and Multiple Workload Domains 

A workload domain is a group ESXi hosts with one or more vSphere clusters, provisioned by SDDC Manager and application ready. In VCF example below, one management domain and two workload domains were deployed. For more details on how to deploy VCF with NetApp storage, check link:https://docs.netapp.com/us-en/netapp-solutions/vmware/vmware-vcf-overview.html[NetApp VCF deployment documentation.] 
image:vmware-vcf-aff-image51.png[VCF one management domain and two workload domains]


== NetApp SnapCenter for vSphere Deployment Steps
To deploy SCV, follow the steps to install and configure SCV and restore a VM and files or directories.

.Deploy NetApp SnapCenter Plug-in for VMware vSphere on each workload domain virtual center 
[%collapsible]
====
...
{nbsp}
+
. Power On OVA to start the SnapCenter Plug-in for VMware vSphere and then click Install VMware tools.
+
...
{nbsp}
+
. Log in to the SnapCenter Plug-in for VMware vSphere management GUI with the admin username and password set at the time of deployment and the MFA token generated using the maintenance console.
...
.Configure SnapCenter Plugin for VMware vSphere
[%collapsible]
====
...
image:vmware-vcf-aff-image55.png[Getting started with SnapCenter for VMWare vSphere]
...
[%collapsible]
====
VMs, VMDKs, files, and folders from backups can be restored. VM can be restored to the original host or an alternate host in the same vCenter Server, or to an alternate ESXi host managed by the same vCenter. Individual files and folders can be restored in a guest file restore session, which attaches a backup copy of a virtual disk and then restores the selected files or folders. Files and folders can also be restored. More details check SnapCenter file and folder restore.
