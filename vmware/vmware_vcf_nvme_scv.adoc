...
= Speed, Scalable and Simple Data Protection for VMware VCP with NVMe-Based NetApp Storage  
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead] 
NVMe over TCP is a network protocol that allows for high-speed data transfer between VMware Cloud Foundation ESXi servers and NetApp All Flash FAS (AFF) and All SAN Array (ASA) storage using the NVMe (Non-Volatile Memory Express) standard, which is particularly beneficial for demanding workloads due to its low latency. NVMe over TCP also enables fast data transfer rates, resulting in quicker backup and restore operations for VMs, datastore and files and folders. NetApp SnapCenter Plug-in for VMware vSphere (SCV) is a powerful tool designed to simplify the backup, restore, and cloning operations for VMware environments. SCV integrates with NetApp storage systems and VMware vSphere to provide application-consistent data protection and efficient storage management.



This document provides deployment steps on how to SCV protect VCF multiple domains with NVMe storage. 
...
The link:https://docs.netapp.com/us-en/ontap-tools-vmware-vsphere/index.html[ONTAP tools for VMware vSphere] (OTV) provides end-to-end life cycle management for virtual machines in VMware environments that use NetApp storage systems. It enables the administrators to manage the storage within the vCenter Server directly and hence simplifies the storage and data management for VMware environments. OTV is optional in this deployment. 
...
.Restore VM, dataore, virtual disk and files or folders 
[%collapsible]
====
VMs, datastores, VMDKs, files, and folders from backups can be restored. VM can be restored to the original host or an alternate host in the same vCenter Server, or to an alternate ESXi host managed by the same vCenter. You can mount a traditional datastore from a backup if you want to access files in the backup. You can either mount the backup to the same ESXi host where the backup was created or to an alternate ESXi host that has the same type of VM and host configurations. You can mount a datastore multiple times on a host. Individual files and folders can also be restored in a guest file restore session, which attaches a backup copy of a virtual disk and then restores the selected files or folders. Files and folders can also be restored. For VMFS environments, SCV uses clone and mount operations with Storage VMotion to perform restore operations. For more details check link:https://docs.netapp.com/us-en/sc-plugin-vmware-vsphere/scpivs44_how_restore_operations_are_performed.html[how the restore operations are performed.] 
...
SCV for VCP on NVMe centralizes data protection, efficiently reduces the time and storage space required for backups using NetApp snapshots, supports large-scale VMware environments with robust backup and replication features and allows granular recovery of entire VMs, specific VMDKs, or individual files.
