...
| VMFS on iSCSI | *Import Tool* | No | Yes | Yes
// | VMFS on NVMe/TCP | No | No | Yes | Yes
| vVol with FC | No | No | No | Yes
| vVol with iSCSI | No | No | No | Yes
...
* *Deploy and use the SnapCenter Plug-in for VMware vSphere to protect and restore VMs in a VI workload domain*. Click link:vmware_vcf_asa_scv_wkld.html
