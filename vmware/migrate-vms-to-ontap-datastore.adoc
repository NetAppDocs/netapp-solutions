...
[%collapsible%open]
==== 
* Refer to the https://blogs.vmware.com/cloud-foundation/2023/01/01/migrating-to-the-express-storage-architecture-in-vsan-8/[Migrating to the Express Storage Architecture in vSAN 8] document for additional details.
====
...
[TIP]
In Cross vCenter migration option, import from target if export option is not available on source. For that procedure, check https://techdocs.broadcom.com/us/en/vmware-cis/vsphere/vsphere/8-0/vcenter-and-host-management-8-0/migrating-virtual-machines-host-management/vmotion-across-vcenter-server-systems-host-management/migrate-a-virtual-machine-from-an-external-vcenter-server-instance-host-management.html[Import or Clone a Virtual Machine with Advanced Cross vCenter vMotion]
====
