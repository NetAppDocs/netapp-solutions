...
[.lead]
In this scenario we will demonstrate how to deploy and use ONTAP Tools for VMware vSphere (10.1 to configure an *NFS datastore* for vSphere 8. 

== Use ONTAP Tools 10.1 to configure NFS datastores for vSphere 8
...
OTV 10.1 deployment documentation starting at https://docs.netapp.com/us-en/ontap-tools-vmware-vsphere-10/deploy/prepare-deployment.html[Prepare to deploy ONTAP tools for VMware vSphere].

=== Deployment Steps
To deploy ONTAP Tools 10.1 and use it to create a vVols and NFS datastore on the VCF management domain, complete the following steps:
...
+
image::vmware-nfs-otv10-image02[DPG - team uplinks]
+
{nbsp}
