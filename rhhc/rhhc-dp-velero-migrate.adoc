...
* The Data Protection Application should be created.

Use the following spec to create the DataProtectionApplication object.
....
spec:
  backupLocations:
    - velero:
...
* Astra Trident must be installed.
* The postgresql app must not be already installed in the postgresql namespace.
* OADP operator must be installed, and the BackupStorage Location must be pointing to the same object storage location where the backup was stored from the first cluster.
