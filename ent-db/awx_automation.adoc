...
.. Provide name of the group for ONTAP, paste the group variables (if any) and click on 'Save'
.. Repeat the process for another group for the Oracle.
.. Then click on the ONTAP group created, go to 'Hosts' sub-menu and click on 'Add New Host'
.. Provide IP address of the ONTAP cluster management IP, paste the host variables (if any) and click on 'Save'
.. This process will need to repeated for the Oracle group and Oracle host(s) management IP/Hostname
...
.. Click 'Back to Credential and click on 'Add'
.. Enter the name and organization details for Oracle
.. Select the 'Credential Type' 'Machine'
.. Under 'Type Details' Enter the Username, Password for the Oracle Hosts
...
<<<<<<< HEAD
++++
<div>&#160;2. Back to AWX or Tower and go to 'Resources' → 'Hosts' and select and open your Oracle server configuration page
</div>
++++

=======
.. Back to AWX or Tower and go to 'Resources' → 'Hosts' and select and open your Oracle server configuration page
>>>>>>> 5888ca9a29ba7ac6310f4bf3bd3a9394713a68f8
...
.. Checkbox 'Prompt on Launch' in the 'Job Tags' field.
.. Click on 'Save'
. Launch job template
.. Go to 'Resources' → 'Templates'
.. Click on the desired template and then click on 'Launch'
.. When prompted on launch for 'Job Tags' type in requirements_config, you may need to click on Create "job tag" line right below requirements_config to get the job tag entered

NOTE: The requirements_config will ensure you have the correct libraries to run the other roles

[start=4]
.. Click 'Next' and then 'Launch' to start the job.
.. Click on 'View' → 'Jobs' to monitor the job output and progress
.. When prompted on launch for 'Job Tags' type in ontap_config, you may need to click on Create "job tag" line right below ontap_config to get the job tag entered
.. Click 'Next' and then 'Launch' to start the job.
.. Click on 'View' → 'Jobs' to monitor the job output and progress
.. Once the ontap_config role has completed, run again for the linux_config
.. Go to 'Resources' → 'Templates'
.. Click on the desired template and then click on 'Launch'
.. When prompted on launch for 'Job Tags' type in linux_config, you may need to click on Create "job tag" line right below linux_config to get the job tag entered
.. Click 'Next' and then 'Launch' to start the job.
.. Click on 'View' → 'Jobs' to monitor the job output and progress
.. Once the linux_config role has completed, run again for the oracle_config
.. Go to 'Resources' → 'Templates'
.. Click on the desired template and then click on 'Launch'
.. When prompted on launch for 'Job Tags' type in oracle_config, you may need to click on Create "job tag" line right below oracle_config to get the job tag entered
.. Click 'Next' and then 'Launch' to start the job.
.. Click on 'View' → 'Jobs' to monitor the job output and progress
