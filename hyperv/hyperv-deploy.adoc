...
NOTE: When iSCSI is used with dedicated NICs, then using any teaming solution is not recommended and MPIO/DSM should be used. But when iSCSI is used with shared NICs, those shared NICs can be teamed. 
...
+ 
image:hyperv-deploy-image02.png[]
+
...
+
image:hyperv-deploy-image03.png[]
...
+
image:hyperv-deploy-image04.png[]
+
...
+
image:hyperv-deploy-image05.png[]

Create Highly available virtual machines on the SMB share
...
+
image:hyperv-deploy-image06.png[]
...
+
image:hyperv-deploy-image07.png[]
...
.Capacity monitoring for overprovisioned CSV volumes
[%collapsible]
====
To ensure your storage system operates efficiently, accurately monitor available space, it is critical not to overprovision your CSV volume or SMB share beyond the total SSD capacity of your FSx for NetApp ONTAP file system. This approach allows for straightforward storage space management within the Hyper-V.

Contact NetApp SA’s to plan and size storage and host requirements accurately. We recommend identifying storage performance requirements before finalizing the CSV layout for production deployments.
==== 

.Migration
[%collapsible]
====
One of the most common use cases in the current market condition is migration. Customers can use VMM Fabric or other third-party migration tools to migrate VMs, these tools use host level copy to move data one source to destination platform which can be time consuming depending on number of virtual machines that are in scope of migration.
...
For additional information about migration using Flexclone and PowerShell, see Appendix section
====
...
<Screenshot here>
image:hyperv-deploy-image08.png[]
====
...
+
image:hyperv-deploy-image09.png[]
...
+
image:hyperv-deploy-image12.png[]

. Click Next and the Save.
+
image:hyperv-deploy-image13.png[]
...
+
image:hyperv-deploy-image14.png[]
+
...
+
image:hyperv-deploy-image15.png[]
...
+
image:hyperv-deploy-image16.png[]
+
...
NOTE: NetApp SnapManager for Hyper-V is currently not supported. An updated version will be available in the future.
