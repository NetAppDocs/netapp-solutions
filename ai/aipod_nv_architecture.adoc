...
Each DGX H100 system is provisioned with two dual-ported ConnectX-7 cards for ethernet and storage traffic, and for this solution both ports on each card are connected to the same switch. One port from each card is then configured into a LACP MLAG bond with one port connected to each switch, and VLANs for in-band management, client access, and user-level storage access are hosted on this bond. 

The other port on each card is used for connectivity to the AFF A900 storage systems, and for this validation those ports were configured in an active/passive bond to deliver high availability in a dedicated RoCE storage VLAN. This bond supports NFS over RDMA with NFS v4.1 pNFS and Session Trunking for high-speed parallel access to all storage nodes in the cluster. For deployments that do not require RDMA customers can also leverage LACP bonding on the storage ports to deliver high availability and additional bandwidth. 
...
link:aipod_nv_storage.html[Next: NetApp AIPod with NVIDIA DGX Systems - Solution Validaton and Sizing Guidance]