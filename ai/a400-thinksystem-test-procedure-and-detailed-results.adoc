...
image:a400-thinksystem-image8.png[This figure shows the ResNet50 Scaleout or the Aggregate Image per second. Compute nodes are connected in 100 GbE.]

image:a400-thinksystem-image9.png[This figurwe shows the ResNet50 for Independent jobs. Runtime in minutes. Compute nodes. 100 GbE. Both combined concurrent training model and singular training model.]

For the independent training models, we used eight GPUs from each server. The average runtime for the training models was 35 minutes and 9 seconds. The individual runtimes were 34 minutes and 32 seconds, 36 minutes and 21 seconds, 34 minutes and 37 seconds, 35 minutes and 25 seconds, and 34 minutes and 31 seconds, respectively. The average images per second for the training model were 22, 573, and the individual images per second were 21,764, 23,438, 22,556, 22,564, and 22,547, respectively.

Based on our validation, one independent training model with a NetApp data runtime was 34 minutes and 54 seconds with 22,231 images/sec. One independent training model with a local data (DAS) runtime was 34 minutes and 21 seconds with 22,102 images/sec. During those runs,  the average GPU utilization was 96%, as reported by nvidia-smi. Note that this average includes the testing phase, during which GPUs were not used, while CPU utilization was 40% as measured by mpstat. This demonstrates that the data delivery rate was sufficient in each case.
